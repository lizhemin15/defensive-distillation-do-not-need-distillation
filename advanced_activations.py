...
class Softmax_t(Layer):
    ...
    def __init__(self, axis=-1,temperature=1, **kwargs):
        super(Softmax_t, self).__init__(**kwargs)
        self.supports_masking = True
        self.axis = axis
        self.temperature=temperature

    def call(self, inputs):
        return activations.softmax(inputs/self.temperature, axis=self.axis)

    def get_config(self):
        config = {'axis': self.axis}
        base_config = super(Softmax_t, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
